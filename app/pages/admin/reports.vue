  ...
  <div class="flex flex-col h-full bg-surface-950 text-surface-50 min-h-0 overflow-hidden">
    ...
    <main v-else class="flex-1 overflow-y-auto">
      ...
      <div v-if="activeTab === 'overview'" class="p-6 space-y-6">
        ...
        <!-- 💳 ยอดแยกตามประเภทชำระ -->
        <div class="bg-surface-900 p-6 rounded-2xl border border-surface-800">
          <h3 class="text-sm font-bold mb-4">💳 ยอดแยกตามประเภทชำระ</h3>
          <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
            <div v-for="pm in paymentBreakdown" :key="pm.method"
              class="bg-surface-800/60 border border-surface-700 rounded-xl p-4">
              <div class="flex items-center gap-2 mb-2">
                <span class="text-xl">{{ pm.icon }}</span>
                <span class="text-xs font-semibold text-surface-300">{{ pm.label }}</span>
              </div>
              <div class="text-xl font-black text-surface-50">฿{{ pm.total.toLocaleString() }}</div>
              <div class="text-[10px] text-surface-500 mt-1">{{ pm.count }} บิล · เฉลี่ย ฿{{ pm.count > 0 ? Math.round(pm.total / pm.count).toLocaleString() : 0 }}/บิล</div>
              <div class="text-[10px] text-primary-400 mt-0.5 font-semibold">
                {{ summary.revenue > 0 ? ((pm.total / summary.revenue) * 100).toFixed(1) : '0.0' }}%
              </div>
            </div>
            <div v-if="paymentBreakdown.length === 0" class="col-span-5 text-center text-surface-500 py-6 text-sm">ไม่มีข้อมูล</div>
          </div>
        </div>
      </div>
...
const paymentBreakdown = ref<{ method: string; label: string; icon: string; total: number; count: number }[]>([])
...
async function loadData() {
  ...
  try {
    ...
    paymentBreakdown.value = Object.values(pmMap).sort((a, b) => b.total - a.total)
  } finally {
