...
function generatePrompt() {
  ...
  return `
    ...
    2. "กำไรสินค้า GP" (Gross Profit) คือ ยอดขายหักต้นทุนวัตถุดิบ (COGS) สำหรับวิเคราะห์สินค้า
    
    ### 📊 ข้อมูลสรุปทางการเงิน (สำคัญ: ให้ใช้ตัวเลขนี้ในการวิเคราะห์เท่านั้น ห้ามคำนวณเลขใหม่เอง):
    ...
    - **รายจ่ายปันส่วนตามจริงสำหรับช่วงนี้ (Allocated Expenses): ฿${(dailyAvgExpense || totalExpenses || 0).toLocaleString()}**
    ...
    กฎเหล็กสำหรับคุณ:
    1. ห้ามคำนวณตัวเลขกำไรสุทธิใหม่เองเด็ดขาด ให้ใช้ค่า **Net Profit** ที่กำหนดให้ข้างต้นเท่านั้น
    2. ห้ามอ้างอิงถึงรายจ่ายก้อนรวมของทั้งเดือนหากยอดนั้นไม่ได้ระบุไว้ในสรุปข้างต้น
