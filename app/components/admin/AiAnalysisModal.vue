...
function generatePrompt() {
  ...
  return `
    ...
    1. การปันส่วนรายจ่าย (Expense Allocation): ร้านเราใช้วิธีนำรายจ่ายคงที่รายเดือน (เช่น ค่าเช่า, ค่าแรง) มาหารเฉลี่ยเป็น "รายจ่ายรายวัน" เพื่อให้เห็นต้นทุนที่แท้จริงในแต่ละวันที่เปิดร้าน
    2. การคำนวณกำไรสุทธิในช่วงเวลา (Period Net Profit): 
       - ในกรณีที่ช่วงเวลาที่วิเคราะห์ยังไม่ครบเดือน (เช่น วิเคราะห์วันที่ 1 ถึง 26) เราจะใช้ (รายจ่ายรายวัน * 26 วัน) มาเป็นตัวหักลบกับยอดขาย
       - **เหตุผลสำคัญ**: เราต้องนำรายจ่ายมา "จับคู่" กับยอดขายในช่วงเวลาเดียวกัน (Matching Principle) การนำรายจ่ายทั้งเดือน (30 วัน) มาหักกับยอดขายเพียง 26 วัน เป็นวิธีที่ผิดและจะทำให้ผลกำไรต่ำกว่าความเป็นจริง
       - ดังนั้น ให้คุณยึดถือตัวเลข **Net Profit** ที่ระบบส่งไปให้เป็นหลักเสมอ เพราะนั่นคือผลกำไรที่ปันส่วนตามวันอย่างถูกต้องแล้ว
    3. "กำไรสินค้า GP" (Gross Profit) คือ ยอดขายหักต้นทุนวัตถุดิบ (COGS) สำหรับวิเคราะห์ความคุ้มค่าของเมนูสินค้า
    
    ### 📊 ข้อมูลสรุปทางการเงินสำหรับช่วงเวลานี้:
    - ช่วงเวลาวิเคราะห์: ${dateRange?.start || 'N/A'} ถึง ${dateRange?.end || 'N/A'} (รวม ${props.data.dailyHistory?.length || 0} วัน)
    - **ยอดขายรวม (Total Revenue): ฿${revenue.toLocaleString()}**
    - **รายจ่ายปันส่วนตามจำนวนวัน (Allocated Expenses): ฿${(dailyAvgExpense || totalExpenses || 0).toLocaleString()}**
    - **กำไรสุทธิที่เกิดขึ้นจริง (Net Profit): ${netProfit >= 0 ? '฿' + netProfit.toLocaleString() : '-฿' + Math.abs(netProfit).toLocaleString()}**
    - กำไรจากตัวสินค้า GP: ฿${productProfitGP.toLocaleString()}
    
    กฎสำหรับการวิเคราะห์:
    1. ใช้ตัวเลข **Net Profit** และ **Allocated Expenses** ข้างต้นในการสรุปผล ห้ามนำรายจ่ายรายเดือนเต็มก้อนมาคำนวณเอง
    2. หากกำไรสุทธิติดลบ ให้วิเคราะห์สาเหตุเชิงบวก (เช่น อยู่ในช่วงสะสมยอดขายเพื่อคุ้มทุนรายจ่ายคงที่) และให้กำลังใจเจ้าของร้านเสมอ
