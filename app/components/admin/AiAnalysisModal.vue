...
function generatePrompt() {
  ...
  return `
    ...
    1. "กำไรสุทธิ" (Net Profit) คือ ยอดขายหักรายจ่ายปันส่วนตามวัน (Allocated Expenses) สำหรับช่วงวันที่เลือกเท่านั้น
    2. "รายจ่ายปันส่วนตามวัน" (Allocated Expenses) คำนวณจาก (รายจ่ายรวมทั้งเดือน / จำนวนวันในเดือนนั้น) * จำนวนวันที่เลือกวิเคราะห์
    3. "กำไรสินค้า GP" (Gross Profit) คือ ยอดขายหักต้นทุนวัตถุดิบ (COGS) สำหรับวิเคราะห์สินค้า
    
    ### 📊 ข้อมูลสรุปทางการเงิน (สำคัญ: ให้ใช้ตัวเลขนี้ในการวิเคราะห์เท่านั้น ห้ามคำนวณย้อนกลับด้วยรายจ่ายรวมทั้งเดือน):
    - ช่วงเวลาวิเคราะห์: ${dateRange?.start || 'N/A'} ถึง ${dateRange?.end || 'N/A'}
    - **ยอดขายรวม (Total Revenue): ฿${revenue.toLocaleString()}**
    - **รายจ่ายปันส่วนสำหรับช่วงเวลานี้ (Allocated Expenses for Period): ฿${(dailyAvgExpense || totalExpenses || 0).toLocaleString()}**
    - **กำไรสุทธิสำหรับช่วงเวลานี้ (Net Profit for Period): ${netProfit >= 0 ? '฿' + netProfit.toLocaleString() : '-฿' + Math.abs(netProfit).toLocaleString()}** (วิเคราะห์ตามยอดนี้เท่านั้น)
    - รายจ่ายรวมทั้งเดือนที่บันทึกไว้ (Reference - Full Monthly Budget): ฿${(props.data as any).actualTotalExpenses?.toLocaleString() || '0'}
    - กำไรจากตัวสินค้า GP: ฿${productProfitGP.toLocaleString()}
    
    กฎเหล็กสำหรับคุณ:
    1. ในการสรุป "สถานะกำไร" ให้ยึดตาม **Net Profit for Period** ที่กำหนดให้ข้างต้นเท่านั้น
    2. ห้ามนำรายจ่ายรวมทั้งเดือน (Full Monthly Budget) มาหักลบกับยอดขายโดยตรง หากช่วงเวลาที่วิเคราะห์ยังไม่ครบเดือน
    3. หากกำไรสุทธิติดลบ ให้วิเคราะห์ถึงสาเหตุและให้กำลังใจเจ้าของร้าน
    
    ${isMonthly ? 'เน้นการวิเคราะห์ความคุ้มค่าเทียบกับรายจ่ายปันส่วนรายวัน และสัดส่วนกำไรสุทธิที่เกิดขึ้นจริงจนถึงปัจจุบัน' : 'เน้นการวิเคราะห์กำไรต่อสินค้าและการเพิ่มยอดขายรายวัน'}
